...
def minimal_yaml(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.chdir(tmp_path)  # isolate from real .env in project root
    data = {
        "provider": "remote",
...
def test_api_key_set(minimal_yaml: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_validate_provider_remote_with_key(minimal_yaml: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("API_KEY", "ok-key")
    settings = load_settings(minimal_yaml)
    # Should not raise
    settings.validate_provider()
...
@pytest.mark.skipif(
    __import__("importlib.util", fromlist=["find_spec"]).find_spec("mlx") is not None,
    reason="mlx is installed in this environment",
)
...
def test_settings_remote_defaults() -> None:
    """Remote backend base_url should default to empty (read from API_URL env var)."""
    s = Settings()
    assert s.remote.base_url == ""
...
def test_settings_log_level_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    s = Settings()
    assert s.log_level.lower() == "warning"
